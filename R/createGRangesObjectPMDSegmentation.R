...
function(m, y.list, num.cores, seqLengths){
  ...
  segList <- mclapply(chrs, function(chr.sel) {
    ...
    segChr <- GRanges(seqnames=chr.sel, IRanges(start=c(1,cumsum(runLength(segNt))[-nrun(segNt)]+1), end=cumsum(runLength(segNt))), strand="*", type=c("notPMD","PMD")[runValue(segNt)], nCG=runLength(segCG), seqlengths=seqLengths)
    segChr
