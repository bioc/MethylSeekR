...
Here, \texttt{GRangesFilename} is the filename for the GRanges object and the
\texttt{TableFilename} the filename for the tab-delimited file (only one of
the two filenames is required).
